...
ratings_count = FOREACH ratings_grouped GENERATE
  ...
  COUNT(ratings.rating) AS number_votes, $0 AS prod_id;

all_rating = ORDER ratings_count BY prod_rating DESC, number_votes DESC;
...
rmf /user/grupo6d4/tmp/better_products_number_evaluations -- remove old elements
store better into '/user/grupo6d4/tmp/better_products_number_evaluations';
